...
from second_step.second_mae_interface import VideoMAEInference
from second_step.second_mae_config import MaeSecondConfig
from collections import Counter

class SecondSliceInference:
    ...
    def get_majority_vote(self, predictions):
        ...
        # 特殊情况：如果 predictions 长度等于2，应用特殊规则
        if len(predictions) == 2:
            # 获取两个模型的预测结果
            class1, score1, _ = predictions[0]
            class2, score2, _ = predictions[1]

            # 规则1: 两个模型都是 normal
            if class1 == 'normal' and class2 == 'normal':
                final_class = 'normal'
                final_score = max(score1, score2)
                return final_class, final_score

            # 规则2: 两个模型都是 zhuyili_no_focus
            if class1 == 'zhuyili_no_focus' and class2 == 'zhuyili_no_focus':
                final_class = 'zhuyili_no_focus'
                final_score = max(score1, score2)
                return final_class, final_score

            # 规则3: 一个是 normal，另一个是 zhuyili_no_focus，取置信度最高的
            if ((class1 == 'normal' and class2 == 'zhuyili_no_focus') or
                    (class1 == 'zhuyili_no_focus' and class2 == 'normal')):
                if score1 >= score2:
                    final_class = class1
                    final_score = score1
                else:
            ...
            # 模型2 闭眼分数很高，模型1 分数不高，输出biyan
            if class2 == 'biyan' and score2 >= 0.99 and score1 < 0.8:
                final_class = class2
                final_score = score2
                return final_class, final_score

            # 规则4: 模型2结果是 biyan，取模型1的结果
            if class2 == 'biyan':
                final_class = class1
                final_score = score1
                return final_class, final_score

            # 规则5: 模型2结果不是 biyan，模型1是 biyan，取模型2的结果
            if class2 != 'biyan' and class1 == 'biyan':
                final_class = class2
                final_score = score2
                return final_class, final_score
    ...
    def infer_video_slices(self, frames, output_dir=None):
    ...
    #         cls_cn = class_names_cn.get(pred['class'], pred['class'])
    ...
    #
    #     # 获取biyan和zhuyili_no_focus类别的统计（不包括normal）
    #     abnormal_classes = ['biyan', 'zhuyili_no_focus']
    #     abnormal_counts = {cls: count for cls, count in class_counts.items() if cls in abnormal_classes}
    #
    #     # 条件1：如果全是normal，则输出normal
    #     if class_counts.get('normal', 0) == total_windows:
    #         print(f"\n决策结果(条件1): 所有窗口都是normal，输出 normal")
    #         return 'normal', class_scores.get('normal', 0.0)
    #
    #     # 条件2：如果有biyan或者zhuyili_no_focus类别
    #     if abnormal_counts:
    #         # 找出出现次数最多的异常类别
    #         max_count = max(abnormal_counts.values())
    #         most_freq_abnormal = [cls for cls, count in abnormal_counts.items() if count == max_count]
    #
    #         if len(most_freq_abnormal) == 1:
    #             # 只有一个类别出现次数最多，直接返回
    #             pred_cls = most_freq_abnormal[0]
    #             cls_cn = class_names_cn.get(pred_cls, pred_cls)
    ...
    #             cls_cn = class_names_cn.get(pred_cls, pred_cls)
    #             print(
    #                 f"\n决策结果(条件2): {most_freq_abnormal}出现次数相同({max_count}/{total_windows})，取置信度最高的 {pred_cls} ({cls_cn})")
    #             return pred_cls, max_score
    #
    #     # 条件3：没有biyan和zhuyili_no_focus，但有normal（部分normal的情况）
    #     print(f"\n决策结果(条件3): 没有检测到biyan或zhuyili_no_focus，输出 normal")
    #     return 'normal', class_scores.get('normal', 0.0)
    ...
    def stage1_decision_ensemble(self, ensemble_predictions):
        ...
        if abnormal_counts:
            ...
            if len(most_freq_abnormal) == 1:
                ...
                # 出现次数一样多，特殊处理：如果biyan和zhuyili_no_focus都是1次，输出zhuyili_no_focus
                if set(most_freq_abnormal) == {'biyan', 'zhuyili_no_focus'} and max_count == 1:
                    pred_cls = 'zhuyili_no_focus'
                    cls_cn = class_names_cn.get(pred_cls, pred_cls)
                    print(f"\n决策结果(条件2): biyan和zhuyili_no_focus都是1次，优先输出 {pred_cls} ({cls_cn})")
                    return pred_cls, class_scores.get(pred_cls, 0.0)
                else:
                    # 其他情况，取置信度最高的
                    max_score = -1
                    pred_cls = None
